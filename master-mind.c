...
void initSeq() {
  ...
  //a loop that fills sequence with random values
...
void showSeq(int *seq) {
  /* *  COMPLETE the code here  *  */
  printf("Secret: ");
  //shows the sequence required format
...
int /* or int* */ countMatches(int *seq1, int *seq2) {
  ...
  //counts when numbers match at the same index
  for(int i = 0; i < SEQL; i++){
    if(seq1[i] == seq2[i]){
      exact++;
      used1[i] = usec2[i] = 1;
  ...
  //counts when numbers match but at different indexes
  for(int i = 0; i < SEQL; i++) {
    if(!used1[i]) {
        for(int j = 0; j < SEQL; j++) {
            if(!usec2[j] && seq1[i] == seq2[j]) {
                approx++;
                usec2[j] = 1;
                break;
            }
...
//combine exact and approx matches into single 32-bit return value
return (approx << 16) | exact;

}
/* show the results from calling countMatches on seq1 and seq2 */
void showMatches(int code, int *seq1 /* only for debugging */, int *seq2 /* only for debugging */, int lcd_format /* optional, to control layout */) {
  /* ***  COMPLETE the code here  ***  */
  // Shows the exact matches with green LED
  int exact = code & 0xFFFF; // Get lower 16 bits for exact matches
  int approx = (code >> 16) & 0xFFFF; // Get upper 16 bits for approximate matches
  ...
  // Red LED blinks once as a break between exact and approximate matches
  writeLED(gpio, LED2, HIGH);
  usleep(DELAY * 1000);
  writeLED(gpio, LED2, LOW);
  usleep(DELAY * 1000);

  // Shows approximate matches with green LED
...
//takes the number 123 and converts to sequence [1,2,3]
void readSeq(int *seq, int val) {
  /* *  COMPLETE the code here  *  */
  int i = SEQL - 1; //starts from the end sequence
  ...
  //parses through each digit and stores it in the sequence
  while(val > 0 && i >=0){
    seq[i] = val % 10; //gets rightmost digit
    val = val/10; //moves on to the next digit
    i--;
  }

  //if there are remaining positions, fill with zeros
  while(i >= 0){
    seq[i] = 0;
    i--;
  }
...
//when testing instead of pressing physical button for  colours users will be able to use the keyboard 1,2,3
int readNum(int max) {
  /* *  COMPLETE the code here  *  */
  int num;
  int c; 

  //reads characters until newline or end of the input
  while((c = getchar()) != '\n' && c ){
    //if character is a digit (0-9), convert it to its numeric value and add to total
    if(c >= '0' && c <= '9'){
      num = num * 10 + (c - '0');//multiply current number by 10 and add new digit
    }
  }
  
  //if input is between 1 and max, return the number, otherwise return error code -1
  if(num > 0 && num <= max){
    return num;
  }
  return -1;
...
void timer_handler (int signum) {
  ...
  // Check if time difference exceeds timeout
  if ((stopT - startT) > TIMEOUT) {
      timed_out = 1;  // Set global flag
  }
}
...
//	mode which sends the commands as 2 x 4-bit values.
  ...
  lcdClear(lcd);
  lcdPosition(lcd, 0, 0);
  lcdPuts(lcd, "My name is Girma");
  lcdPosition(lcd, 0, 1);
  lcdPuts(lcd, "My name is Miski");
  printf("My name is Girma\n");
  printf("My name is Miski\n");

//to show LED blinks for the name
const char* name = "Girma";
int nameLength = 5; 

pinMode(gpio, 5, OUTPUT);   
pinMode(gpio, 26, OUTPUT);
digitalWrite(gpio, 5, 0);
digitalWrite(gpio, 26, 0);

for(int i = 0; i < nameLength; i++) {
    char c = name[i];
    
    //checks if character is any vowel (a,e,i,o,u) in either case
    if(c == 'a' || c == 'e' || c == 'i' || c == 'o' || c == 'u' ||
       c == 'A' || c == 'E' || c == 'I' || c == 'O' || c == 'U') {
        //blinks green for vowels 
        digitalWrite(gpio, 26, 1);
        delay(600);
        digitalWrite(gpio, 26, 0);
    } else {
        //blinks red for constants
...
//adds a small delay before displaying the second name
delay(1000);

const char* name2 = "Miski";
int name2Length = 5;

for(int i = 0; i < name2Length; i++) {
    char c = name2[i];
    
    //check if character is any vowel (a,e,i,o,u) in either case
    if(c == 'a' || c == 'e' || c == 'i' || c == 'o' || c == 'u' ||
       c == 'A' || c == 'E' || c == 'I' || c == 'O' || c == 'U') {
        //blinks green for vowels 
        digitalWrite(gpio, 26, 1);
...
//turns off leds for pins 5 and 26
digitalWrite(gpio, 5, 0);
digitalWrite(gpio, 26, 0);

//displays game intro message
delay(2000);
lcdClear(lcd);
lcdPosition(lcd, 0, 0);
lcdPuts(lcd, "Let's Play!");
printf("Let's Play\n");
delay(2000);
lcdClear(lcd); 
...
lcdClear(lcd);
lcdPosition(lcd, 0, 0);
lcdPuts(lcd, "You have only");
lcdPosition(lcd, 0, 1); 
lcdPuts(lcd, "5 attempts");
...
//initialize the secret sequence here
if (!opt_s) {  //randomised only if -s was not used
  initSeq();
  if (debug) {
...
while (!found && attempts < 5) {
  ...
  //clears the lcd from previous round
  lcdClear(lcd);

  //prints the round number on the terminal
  printf("Round: %d\n", attempts);

  //prints the number of attempts on the next line
  char roundString[32];
  ...
  while (turn < 3)
  {
      ...
      lcdPosition(lcd, 0, 1);
      sprintf(buf, "Position %d", turn);
      lcdPuts(lcd, buf);
      printf("%s\n", buf);
      delay(1000);

      //initialize button press counting
      buttonPressCount = 0;

      lcdClear(lcd);
      lcdPosition(lcd, 0, 0);
      sprintf(buf, "Presses: %d", buttonPressCount);
      lcdPuts(lcd, buf);
      printf("%s\n", buf);

      //sets a 5-second time window
      time_t startTime = time(NULL);
      time_t endTime = startTime + 5; 
  
      //handles button presses within the time window
      while (time(NULL) < endTime)
      {
        if (readButton(gpio, pinButton) == LOW && buttonPressCount < 3) {  
          delay(50);  // Debounce
          while (readButton(gpio, pinButton) == LOW) {
            delay(10);
          }
          buttonPressCount++;  
              //blinks LED briefly to show button press feedback
              digitalWrite(gpio, LED, HIGH);
              delay(200);
              digitalWrite(gpio, LED, LOW);
      ...
      //stores the number of presses in the sequence
      attSeq[turn - 1] = buttonPressCount;
      
      //visual feedback for end of input window
      digitalWrite(gpio, LED2, HIGH);
      delay(500);
      digitalWrite(gpio, LED2, LOW);
      
      //shows final count for this position
      lcdClear(lcd);
      lcdPosition(lcd, 0, 0);
      sprintf(buf, "Position %d:", turn);
      lcdPuts(lcd, buf);
      printf("%s\n", buf);
      lcdPosition(lcd, 0, 1);
      sprintf(buf, "Entered: %d", buttonPressCount);
      lcdPuts(lcd, buf);
      printf("%s\n", buf);
      delay(1000);
  }
  
    //checks if the sequences matches with the secret one
    int matches = countMatches(attSeq, theSeq);
    int exact = matches & 0xFFFF;
    int approx = (matches >> 16) & 0xFFFF;
  ...
  //it blinks green LED for exact matches (only if there are any)
  for (i = 0; i < exact; i++) {
      digitalWrite(gpio, LED, HIGH);
      delay(500);
      digitalWrite(gpio, LED, LOW);
  ...
  //it blinks red LED exactly 1 times as a separator
  for (i = 0; i < 1; i++) {
      digitalWrite(gpio, LED2, HIGH);
      delay(1000);
      digitalWrite(gpio, LED2, LOW);
  ...
  //it blinks green LED for approximate matches (only if there are any)
  for (i = 0; i < approx; i++) {
      digitalWrite(gpio, LED, HIGH); 
      delay(500);
      digitalWrite(gpio, LED, LOW);
  ...
  delay(2000);  //time given for LCD display
  ...
  if (exact == 3) {
    ...
    //clears the sequence for next round
    for (i = 0; i < SEQL; i++)
    {
        attSeq[i] = 0;
    }
  ...
  //it frees memory before exiting
  free(theSeq);
  free(seq1);
  free(seq2);
  free(cpy1);
